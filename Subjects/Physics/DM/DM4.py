...
plt.plot(             ) # Graphe des UD (en V) en fonction de ID (en mA : attention au changement d'unité!) : points expérimentaux représentés par des ronds rouges (à compléter, entre les parenthèses)
...
#Morceau 1
ID1 =  # Tableau contenant les 4 premières valeurs de ID (à compléter)
UD1 =  # Tableau contenant les 4 premières valeurs de UD (à compléter)
...
ED =     # Expression de la fem du modèle de Thévenin équivalent à partir des coefficients de la régression linéaire (à compléter)
RD =     # Expression de la résistance du modèle de Thévenin équivalent à partir des coefficients de la régression linéaire (à compléter)
...
print(f'Fem : ED = {ED:.1f} V') # Affichage de la valeur de ED en notation décimale, avec 1 chiffre après la virgule
print(         ) # Affichage de la valeur de RD en notation décimale, avec 1 chiffre après la virgule (à compléter, entre les parenthèses)
print('\n')
...
UD_mod =    # Concaténation de toutes les valeurs du modèle de UD (à compléter)
plt.plot(        ) #Tracé de la caractéristique linéarisée par morceaux, en tirets (--) rouges (à compléter, entre les parenthèses)
plt.legend(loc='best')
plt.show()

## Cellule 4 : Caractéristique de la pile
E =   # Valeur numérique de la fem de la pile (en V) (à compléter)
R =   # Valeur numérique de la résistance interne de la pile (en Ohm) (à compléter)
Upile =   # Equation de la caractéristique de la pile (à compléter)

plt.plot(          ) #Tracé de la caractéristique de la pile en trait bleu (à compléter, entre les parenthèses)
plt.show()
...
I0 =   # Valeur du courant (en A) permettant d'initier la recherche de zéro effectuée par la fonction fsolve (à compléter)
...
UP =   # Calcul de la valeur de UP, à partir de l'équation de la caractéristique de la pile (à compléter)

#Affichage des coordonnées du point de fonctionnement
print('Coordonnées du point de fonctionnement')
print(        ) # Affichage de la valeur de IP en notation décimale, avec 1 chiffre après la virgule (à compléter, entre les parenthèses)
print(        ) # Affichage de la valeur de UP en notation décimale, avec 1 chiffre après la virgule (à compléter, entre les parenthèses)

plt.plot(          ) # Représentation du point de fonctionnement P par un diamant (D) noir (k) (à compléter, entre les parenthèses)
plt.legend(loc='best')
plt.show()
